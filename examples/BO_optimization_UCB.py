...
def bayesian_optimisation(n_iter=100):
    ...
    while time_simulation < 200:
        ...
        acq_func = UpperConfidenceBound(
            model=model,
            beta=10.0,  # Increased from 9.0 for more exploration
        )
        ...
        min_step = param.agent_radius  # Minimum step length equals agent radius
        
        # Use larger steps if geodesic distance is very small (enforce minimum step length)
        if geodesic_dist < min_step:
            num_steps = 1
        else:
        ...
        time_simulation += end_time - start_time + geodesic_dist / 10
